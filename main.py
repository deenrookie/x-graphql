...
def get_response(url):
    ...
    for item in req_list:
        j += 1
        if j < l:
            if ':' in item and 'Host' not in item:
                header_list = item.split(':')
                if len(header_list) == 2:
                    headers[header_list[0]] = header_list[1].strip()
...
def write_html(result, response_position, host):
    stl = """
<style>
body {
  font-family: Roboto;
...
"""
    with open(host + '.html', 'w') as output_file:
        ...
        if len(response_position) > 0:
            result['data'] = result[response_position]
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d",
                        help="The determine the response data",
                        default='')
    args = parser.parse_args()

    target_url = args.u
    response_position = args.d
    if not target_url:
        print(RED + 'Please input the target url and edit the req.txt' + WHITE)
    else:
        host = target_url.split('/')[2]
        content = get_response(target_url)
        print(GREEN + 'Send request successfully' + WHITE)
        write_html(content, response_position, host)
        print(GREEN + 'Write ' + host + '.html successfully')
